...
class PositionSortOptions(AbstractParser, ApplyPosBinActOptions, ApplySortIdxOptions):
    ...
    def plot_trial_average(self, signal: np.ndarray, output: DataOutput):
        ...
        with plot_figure(output_file, 3, 1, figsize=(6, 12)) as _ax:
            ...
            ax = ax_merge(_ax)[2:]
            plot_fraction_active(ax, signal, track_length=self.track_length, landmarks=self.track_landmarks)
    ...
    def plot_selected_trials(self, signal: np.ndarray, trial_range: tuple[int, int], output: DataOutput):
        ...
        with plot_figure(output_file, 3, n_trials, gridspec_kw={'wspace': 0}) as _ax:
            for i in range(n_trials):
                ...
                #
                ax = ax_merge(_ax)[2:, i]
                plot_fraction_active(ax, signal[:, i, :], track_length=self.track_length,
                                     landmarks=self.track_landmarks)
